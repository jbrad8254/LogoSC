...
2. Select `Bolt`, `Nut`, `Assembly`, or `Profile` with `Part`.
...
- **`TipChamfer`** is the axial/radial amount removed at the shaft tip to ease starting and
  reduce a sharp first thread. It does not alter the nut.
...
- **`HeadType`** selects the positive external shape: `Hex`, `Pan`, `Round`,
  ...
  Headless cuts the selected drive into the shaft end.
